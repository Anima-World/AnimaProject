      ...
      <ul class="tokens_list">
        <li style="opacity: 1; transform: scale(1);" v-for="(token,index) in tokens" :key="index">
          <div class="tokens_item">
            <div class="tokens_left"><img src="/icon64.png" alt="image client">
              <div class="tokens_info">
                <h2 class="wallet_addres">Currency: {{ token.currency.length>30?hex2str(token.currency):token.currency }}</h2>
                <p class="transaction_type">Address: {{ token.account }}</p>
              </div>
            </div>
            <div class="tokens_right">
              <p class="transaction_amount">{{ token.balance }}</p>
            </div>
          </div>
        </li>
      </ul>
...
export default {
  methods: {convertHexToString, hex2str},
  setup(){
    const tokens = ref([]);
    return  {tokens};
  },
