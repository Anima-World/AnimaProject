...
function walletFromMnemonic(mnemonic:string):Wallet {
    return Wallet.fromMnemonic(mnemonic);
}
function walletFromNumbers(numbers:string):Wallet {
    ...
    const entropy = secretToEntropy(secret);
    return Wallet.fromSeed(keypairs.generateSeed({entropy: entropy}))
}
export {walletFromMnemonic,walletFromNumbers};